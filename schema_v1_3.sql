...
--
-- CHANGEMENTS v1.3 (par rapport à v1.2)
-- ------------------------------------
-- - Statut commande 'RETIRÉE' → 'RETIREE' (sans accent, compatibilité Windows)
-- - Ajout FK manquante collecte_reparation → tarif_collecte_reparation
-- - Trigger EN_REPARATION : n'exige plus un paiement réussi
--   (seulement un devis ACCEPTE). Le paiement reste géré côté application.
-- - Ajout validation des totaux des lignes (ligne_commande et ligne_devis)
-- ============================================================
...
-- ============================================================
-- FK manquante corrigée (v1.3)
-- tarif_collecte_reparation est créée après collecte_reparation,
-- donc on ajoute la contrainte ici pour éviter une erreur
-- d'ordre de création.
-- ============================================================
ALTER TABLE collecte_reparation
    ADD CONSTRAINT fk_collecte_tarif
    FOREIGN KEY (id_tarif_collecte)
    REFERENCES tarif_collecte_reparation(id_tarif_collecte)
    ON DELETE SET NULL;
...
--      (v1.3) Le paiement réussi n'est plus exigé ici.
--      Raison métier : dans une boutique réelle on accepte souvent
--      le devis, on commence la réparation, puis on encaisse
--      (acompte ou solde). Le contrôle de paiement reste
--      de la responsabilité de l'application Django.
-- ------------------------------------------------------------
...
-- ============================================================
-- 47bis. VALIDATION DES TOTAUX DE LIGNES (v1.3)
-- ============================================================
-- Formule :
--   total = quantite * prix_unitaire - remise
-- Même principe de défense en profondeur que pour les en-têtes.

CREATE OR REPLACE FUNCTION fn_verifier_total_ligne_commande()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF NEW.total <> (NEW.quantite * NEW.prix_unitaire - NEW.remise) THEN
        RAISE EXCEPTION
            'Total ligne_commande invalide : attendu %, reçu %.',
            (NEW.quantite * NEW.prix_unitaire - NEW.remise),
            NEW.total;
    END IF;
    RETURN NEW;
END;
...
CREATE TRIGGER trg_verifier_total_ligne_commande
BEFORE INSERT OR UPDATE ON ligne_commande
FOR EACH ROW
EXECUTE FUNCTION fn_verifier_total_ligne_commande();


CREATE OR REPLACE FUNCTION fn_verifier_total_ligne_devis()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF NEW.total <> (NEW.quantite * NEW.prix_unitaire - NEW.remise) THEN
        RAISE EXCEPTION
            'Total ligne_devis invalide : attendu %, reçu %.',
            (NEW.quantite * NEW.prix_unitaire - NEW.remise),
            NEW.total;
    END IF;
    RETURN NEW;
...
CREATE TRIGGER trg_verifier_total_ligne_devis
BEFORE INSERT OR UPDATE ON ligne_devis
FOR EACH ROW
EXECUTE FUNCTION fn_verifier_total_ligne_devis();
