...
CREATE TABLE unite_produit (
    ...
    imei                VARCHAR(50),
    type_etat           VARCHAR(20) NOT NULL DEFAULT 'NEUF',
    ...
    CONSTRAINT ck_unite_type_etat
        CHECK (type_etat IN ('NEUF', 'OCCASION')),
...
CREATE TABLE offre_produit (
    ...
    CONSTRAINT ck_offre_occasion_unite
        CHECK (
...
BEGIN
    ...
    IF v_deja_rembourse + NEW.montant > v_paiement THEN
        RAISE EXCEPTION
            'Le remboursement dépasse le montant du paiement %.',
            NEW.id_paiement;
    END IF;
